...
一个 Android 手机优先、同时支持 Windows Chrome/Edge 的私人离线中文小说 PWA。阶段 4.4A 已加入“正文 TXT + 可选章节目录 TXT”的辅助章节识别；ReaderScreen 支持断点续读、目录、滚动/左右翻页、文本锚点、设置与四套主题。没有服务器、账号、云同步、网络 API 或在线正文，小说和目录不会离开当前设备。
...
导入界面明确区分“小说正文 TXT（必选）”和“章节目录 TXT（可选）”。没有目录时完全沿用普通 TXT 解析；提供目录时，目录会复用 UTF-8/GB18030 解码设施，在同一个 Web Worker 中解析为独立的 `ReferenceChapterIndex`，再与正文解析器检测到的边界按章号、规范化标题、前缀证据和单调顺序进行保守对齐。整个过程完全离线，目录文件不会保存成第二本 Book。

目录只是辅助参考，不是绝对真理。EXACT/HIGH 匹配可以用更干净的目录标题校准显示；FUZZY 只有在章号一致、标题前缀可靠且位于段落起点等证据同时成立时，才允许拆开标题与粘连正文。UNRESOLVED 只计入摘要：不创建空 Chapter、不按字数猜边界、不强制让最终章节数等于目录数量。找不到的目录章节保持在相邻正文的原有合并状态，正文独有章节也不会被删除，用户始终可以继续确认导入。

确认页会显示目录章节数、正文原始检测数、高置信/模糊匹配数、未找到目录章节数、正文独有章节数和最终章节数。目录为空、无法读取或内容无法识别时自动退回普通解析，并给出可理解的提示。同一正文即使换了目录，重复检测仍只依据正文 hash，用户可明确选择覆盖重新处理；覆盖前会提示该书阅读进度将重置到开头。
...
阶段 4.4A 已完成可选章节目录解析、宽松顺序对齐、匹配摘要和安全回退。搜索、分类、标签、封面、账号、云同步和 OCR 仍未加入；产品也不包含书签、书城、网络正文、TTS 或复杂翻书动画。浏览器删除站点数据后无法恢复书库；不同 Android WebView/厂商浏览器仍建议用目标设备执行一次安装与长时间阅读测试。
