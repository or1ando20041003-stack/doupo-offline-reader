...
3. 新手机上应该只看到“选择 TXT 文件”的导入页面，不会直接出现小说正文。
...
2. 从手机桌面打开阅读器。
3. 点击 **选择 TXT 文件**。
4. 在 Android 文件选择器中打开 **Download/下载**，选择 `斗破苍穹.txt`。
5. 等待读取、识别编码、清洗、解析和保存完成。大文件可能需要一些时间，请不要中途关闭应用。
6. 完成后会自动进入第一章。以后打开应用会继续上一次阅读位置。
...
3. 从手机桌面重新打开阅读器。
4. 应该直接恢复上次章节和位置；目录、Scroll、Paged 和设置都应该可用。
...
普通程序更新只替换 HTML、CSS、JavaScript 和 Service Worker，不会主动删除 IndexedDB 中的小说与进度。只有主动重新导入、清理网站数据，或卸载时选择清理数据，才可能影响本地小说。
