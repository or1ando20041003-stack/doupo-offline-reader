...
5. 等待读取、识别编码、清洗和章节解析完成。大文件在后台处理，期间会显示当前步骤，请不要中途关闭应用。
6. 在“确认书籍信息”界面核对或修改书名，并检查章节数、正文字符数和编码。此时数据尚未写入书架；点击 **重新选择** 可以安全取消。
7. 点击 **确认导入**，等待“保存数据”完成。返回书架并看到小说卡片后，导入才算完成。
8. 点击 **继续阅读** 进入第一章；以后打开应用先选择书籍，再恢复该书上一次阅读位置。
